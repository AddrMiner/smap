...
impl ModeMethod for PrefixTree6 {
    fn execute(&self) {
        ...
        // 补充扫描状态
        let mut supplement_scan_state = false;
        ...
        'total:loop {
            ...
            if total_send_success >= self.budget { 
                if self.allow_supplement_scan && (!supplement_scan_state) {
                    // 如果允许补足扫描, 并且当前状态 不处于 补足扫描状态
                    ...
                    info!("{} {} {}", SYS.get_info("info", "start_supplement_scan"), SYS.get_info("info", "tar_num"), seeds.len());
                    double_tree_struct.set_targets(seeds, addr_to_seq);

                    supplement_scan_state = true;
                } else { break 'total }
            } else {
                // 更新前缀树

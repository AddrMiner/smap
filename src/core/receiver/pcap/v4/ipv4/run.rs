...
impl PcapReceiver {
    ...
    pub fn run_v4(// 网络接口下标, 用来指定接收器绑定的网络接口
               interface_index:usize,
               base_conf:Arc<BaseConf>, receiver_conf:Arc<ReceiverBaseConf>,
               // 探测模块用来验证信息, 处理数据包
               probe_mod:Arc<ProbeModV4>,
               // 起止地址 和 目标地址数量, 用来初始化位图
               start_ip:u32, end_ip:u32, tar_ip_num:u64, sports:Vec<u16>, tar_ports:Vec<u16>,
            ...
            // 初始化 位图查重器
            let mut bit_map = BitMapV4Port::new(start_ip, end_ip, tar_ip_num, tar_ports);

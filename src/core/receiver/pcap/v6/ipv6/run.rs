...
use crate::SYS;
use crate::tools::check_duplicates::bit_map_v6_port::BitMapV6Port;

impl PcapReceiver {

    pub fn run_v6(// 网络接口下标, 用来指定接收器绑定的网络接口
               interface_index:usize,
               base_conf:Arc<BaseConf>, receiver_conf:Arc<ReceiverBaseConf>,
               // 探测模块用来验证信息, 处理数据包
               probe_mod:Arc<ProbeModV6>,
               // 起始地址 和 目标地址数量, 用来初始化位图
               start_ip:u128, end_ip:u128, tar_ip_num:u64, sports:Vec<u16>, tar_ports:Vec<u16>,
            ...
            // 初始化 位图查重器
            let mut bit_map = BitMapV6Port::new(start_ip, end_ip, tar_ip_num, tar_ports);
            ...
            for packet in receiver.active_capture.iter(Codec) {
                ...
                match packet {
                    ...
                    Ok(packet) => {
                        ...
                        if ip_ver == 6 {
                            // 如果是ipv6的数据包
                            Self::handle_packet_v6(&header,data_link_header,data_link_len,
                                                   net_layer_data,&aes_rand, &mut bit_map, &mut receiver_info,
                                                    &probe, &mut output);
                        }

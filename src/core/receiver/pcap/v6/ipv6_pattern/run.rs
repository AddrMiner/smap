...
impl PcapReceiver {
    ...
    pub fn run_v6_pattern(// 网络接口下标, 用来指定接收器绑定的网络接口
               interface_index:usize,
               base_conf:Arc<BaseConf>, receiver_conf:Arc<ReceiverBaseConf>,
               // 探测模块用来验证信息, 处理数据包
               probe_mod:Arc<ProbeModV6>,
               // 二进制位数量, 基础ip值, 掩码, 片段信息, 用来初始化位图
               ip_bits_num:u32, base_ip_val:u128, mask:u128, parts:Vec<(u32, u32)>, sports:Vec<u16>, tar_ports:Vec<u16>,
            ...
            // 初始化 位图查重器
            let mut bit_map = BitMapV6PatternPort::new(ip_bits_num, base_ip_val, mask, parts, tar_ports);

...
impl PcapReceiver {
    ...
    pub fn run_v6_port_hash(// 网络接口下标, 用来指定接收器绑定的网络接口
                       interface_index:usize,
                       base_conf:Arc<BaseConf>, receiver_conf:Arc<ReceiverBaseConf>,
                       // 探测模块用来验证信息, 处理数据包
                       probe_mod:Arc<ProbeModV6>, hash_set_capacity:Option<u64>, sports:Vec<u16>,
        ...
        // 初始化 哈希集合查重器
        let mut hash_set:AHashSet<(u128, u16)>;
        if let Some(cap) = hash_set_capacity {
            // 如果传入了预设容量
            hash_set = AHashSet::with_capacity(cap as usize);
        } else {
            hash_set = AHashSet::new();
        }
        ...
        for packet in receiver.active_capture.iter(Codec) {
            ...
            match packet {
                ...
                Ok(packet) => {
                    ...
                    if ip_ver == 6 {
                        // 如果是ipv6的数据包
                        Self::handle_packet_v6_port_hash(&header,data_link_header,net_layer_data,&aes_rand,
                                                         &mut hash_set, &mut receiver_info, allow_no_succ, &probe, &mut output);
                    }

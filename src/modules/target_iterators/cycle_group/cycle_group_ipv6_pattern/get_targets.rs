use crate::modules::target_iterators::cycle_group::cycle_group_ipv6_pattern::CycleIpv6Pattern;
...
impl CycleIpv6Pattern {
    ...
    pub fn get_first_ip_port(&mut self) -> (bool, bool, u128, u16) {
        if self.current == self.last {
            // 如果初始值是最后一个
    ...
    /// 获取下一个目标的 ip 和 port
    /// 返回值: 0:是否为<u>非最终值</u>, 1:最终值是否有效, 2:ip地址, 3:端口号
    pub fn get_next_ip_port(&mut self) -> (bool, bool, u128, u16) {
        ...
        loop {
            ...
            if target_val.0 {
                ...
                // 如果是最终值
                return if target_val.1 == u128::MAX {
                    // 如果最终值无效
                    (false, false, 0, 0)
                } else {
                    // 如果最终值有效
